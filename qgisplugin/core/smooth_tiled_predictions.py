...
def split_into_batches(big_batch, batch_size):
    """
    Splits a large batch into smaller batches.

    Parameters:
        big_batch (numpy.ndarray): The input array of shape (N, C, H, W).
        batch_size (int): The desired size of smaller batches.

    Returns:
        list: A list of numpy arrays, each of shape (batch_size, C, H, W) except possibly the last one.
    """
    num_samples = big_batch.shape[0]
    batches = []
    
    for i in range(0, num_samples, batch_size):
        batches.append(big_batch[i:i + batch_size])
...
def _windowed_subdivs(padded_img, window_size, subdivisions, nb_classes, pred_func):
    ...
    gc.collect()
    if __name__ == "__main__":
        print("The number of subdivs are", subdivs.shape)

        # Get the batches...
        BATCH_SIZE = 1
        batches = split_into_batches(subdivs, BATCH_SIZE)
        pred_array = []
        for batch in batches:
            pred_array.append(pred_func(batch))
            gc.collect()
        
        subdivs = np.concatenate(pred_array, axis=0)
        # subdivs = pred_func(subdivs)
        gc.collect()

        # Apply the window spline
        subdivs = np.array([patch * WINDOW_SPLINE_2D for patch in subdivs] )
    else:
        BATCH_SIZE = 5
        print("PROCESSING THIS MANY PATCHES: ", subdivs.shape[0])
        batches = split_into_batches(subdivs, BATCH_SIZE)

        pred_array = []
        for batch in batches:
            print(".",end="")
            pred_array.append(pred_func(batch).detach().numpy())
            gc.collect()
        ...
        # Apply the window spline
        subdivs = np.array([patch.transpose(1, 2, 0) * WINDOW_SPLINE_2D for patch in subdivs] )

    gc.collect()

    # Such 5D array:
    print("a, b, c, d,", a, b, c, d, nb_classes)
...
if __name__ == '__main__':
    ...
    print("The shape is:", input_img.shape)
